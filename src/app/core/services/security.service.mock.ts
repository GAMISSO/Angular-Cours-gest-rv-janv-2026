...
export class SecurityMockService implements ISecurityService {
  ...
  private currentUserSubject = new BehaviorSubject<UserLoginResponse | null>(this.readUserFromLocalStorage());
  private currentUser$: Observable<UserLoginResponse | null> = this.currentUserSubject.asObservable();
  private isAuthenticatedSubject = new BehaviorSubject<boolean>(this.readIsAuthenticatedFromLocalStorage());
  ...
  private readUserFromLocalStorage(): UserLoginResponse | null {
  ...
  private readIsAuthenticatedFromLocalStorage(): boolean {
